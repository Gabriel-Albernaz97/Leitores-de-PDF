...
# IDENTIFICAÇÃO DO CÓDIGO CSLL + TIPO
def identificar_codigo_csll(texto):
    """
    Retorna:
    - 2372 – CSLL Lucro Presumido
    - 6012 – CSLL Lucro Real Trimestral
    - 2484 – CSLL Lucro Real Mensal
    """
    texto = texto.upper()

    codigos_csll = {
        "2372": "Lucro Presumido",
        "6012": "Lucro Real Trimestral",
        "2484": "Lucro Real Mensal",
    }

    padrao = r"(CÓDIGO DE RECEITA|CÓDIGO)?\s*[:\-]?\s*(2372|6012|2484)"
...
def validar(texto):
    ...
    tem_csll = any(p in texto for p in evidencias_csll)
    tem_receita = any(p in texto for p in evidencias_receita)
    tem_proibido = any(p in texto for p in impostos_proibidos)

    codigo_info = identificar_codigo_csll(texto)
    tem_codigo = codigo_info.get("codigo") is not None

    return tem_csll and tem_receita and tem_codigo and not tem_proibido
...
def extrair_razao_social(texto):
    ...
    for i, linha in enumerate(linhas):
        if "RAZÃO SOCIAL" in linha.upper() and i + 1 < len(linhas):
            ...
            razao = re.sub(
                r"\b(CSLL|DARF|RECEITA FEDERAL|RFB)\b.*",
                "",
                razao,
                flags=re.IGNORECASE
            )
...
def extrair_valor_documento(texto):
    texto = texto.replace("\n", " ")
    match = re.search(
        r"(VALOR TOTAL DO DOCUMENTO|VALOR|TOTAIS)[^\d]{0,20}"
        r"(\d{1,3}(?:\.\d{3})*,\d{2})",
        texto,
        re.IGNORECASE
    )
