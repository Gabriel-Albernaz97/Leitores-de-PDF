...
def validar(texto):
    ...
    tem_cofins = any(p in texto for p in evidencias_cofins)
    tem_proibido = any(p in texto for p in impostos_proibidos)

    return tem_receita and tem_cofins and not tem_proibido
...
def extrair_razao_social(texto):
    ...
    for i, linha in enumerate(linhas):
        if "RAZÃO SOCIAL" in linha.upper() and i + 1 < len(linhas):
            ...
            razao = re.sub(
                r"\b(COFINS|DARF|RFB|RECEITA FEDERAL)\b.*",
                "",
                razao,
                flags=re.IGNORECASE
            )
...
def extrair_vencimento(texto):
    match = re.search(
        r"VENCIMENTO[:\s]+(\d{2}/\d{2}/\d{4})|PAGAR ATÉ[:\s]+(\d{2}/\d{2}/\d{4})",
        texto,
        re.IGNORECASE
    )
...
def extrair_valor_documento(texto):
    texto = texto.replace("\n", " ")
    match = re.search(
        r"(TOTAL|VALOR TOTAL|TOTAIS|VALOR TOTAL DO DOCUMENTO)[^\d]{0,20}"
        r"(\d{1,3}(?:\.\d{3})*,\d{2})",
        texto,
        re.IGNORECASE
    )
...
# CÓDIGO COFINS + TIPO
def extrair_codigo_cofins(texto):
    match = re.search(r"\b(2172|2176|5856)\b", texto)

    if not match:
        return {
            "codigo": None,
            "tipo": None,
            "alerta": "⚠️ Código COFINS não encontrado – Acionar o TI"
        }
...
def extrair(texto, caminho_pdf):
    codigo_info = extrair_codigo_cofins(texto)
